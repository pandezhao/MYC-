...
class city{
	...
	city* red_quarter_city = NULL;
	city* blue_quarter_city = NULL;
	public:
		friend game;
...
class blue_quarter:public city{
	...
	public:
		...
		virtual void lion_escape(){
			if(city::blue_warrior -> get_name() == "lion" && city::blue_warrior -> escape()){
				timer.zhanshi();
...
int main(){
	...
	for (int i=0;i<t;i++){
		...
		int time=0;
		game newgame(M,N,dragon_life,ninja_life,iceman_life,lion_life,wolf_life,dragon_attack,ninja_attack,iceman_attack,lion_attack,int wolf_attack,wolf_attack);
		zhen = &newgame;
		timer.reset();
		newgame.start();
		delete zhen;
